...
public abstract class BaseEncoding {
   ...
   * Writer}.
   */
  @vendor.google.common.annotations.GwtIncompatible // Writer,OutputStream
  public abstract OutputStream encodingStream(Writer writer);

  /**
   * Returns a {@code ByteSink} that writes base-encoded bytes to the specified {@code CharSink}.
   */
  @vendor.google.common.annotations.GwtIncompatible // ByteSink,CharSink
  public final vendor.google.common.io.ByteSink encodingSink(final CharSink encodedSink) {
   ...
   */
  @vendor.google.common.annotations.GwtIncompatible // Reader,InputStream
   ...
   * CharSource}.
   */
  @vendor.google.common.annotations.GwtIncompatible // ByteSource,CharSource
  public final vendor.google.common.io.ByteSource decodingSource(final CharSource encodedSource) {
  ...
  private static final class Alphabet {
    ...
    Alphabet(String name, char[] chars) {
      ...
      byte[] decodabet = new byte[vendor.google.common.base.Ascii.MAX + 1];
    ...
    boolean canDecode(char ch) {
      return ch <= vendor.google.common.base.Ascii.MAX && decodabet[ch] != -1;
    }

    int decode(char ch) throws DecodingException {
      if (ch > vendor.google.common.base.Ascii.MAX) {
        throw new DecodingException("Unrecognized character: 0x" + Integer.toHexString(ch));
      }
      int result = decodabet[ch];
      if (result == -1) {
        if (ch <= 0x20 || ch == vendor.google.common.base.Ascii.MAX) {
          throw new DecodingException("Unrecognized character: 0x" + Integer.toHexString(ch));
        } else {
    ...
    private boolean hasLowerCase() {
      for (char c : chars) {
        if (vendor.google.common.base.Ascii.isLowerCase(c)) {
          return true;
        }
      }
      return false;
    ...
    private boolean hasUpperCase() {
      for (char c : chars) {
        if (vendor.google.common.base.Ascii.isUpperCase(c)) {
          return true;
        }
      }
      return false;
    ...
    Alphabet upperCase() {
      if (!hasLowerCase()) {
        ...
        for (int i = 0; i < chars.length; i++) {
          upperCased[i] = vendor.google.common.base.Ascii.toUpperCase(chars[i]);
        }
    ...
    public boolean equals(@Nullable Object other) {
      if (other instanceof StandardBaseEncoding) {
        ...
        return this.alphabet.equals(that.alphabet)
            && vendor.google.common.base.Objects.equal(this.paddingChar, that.paddingChar);
      }
      return false;
    }
  ...
  @vendor.google.common.annotations.GwtIncompatible // Writer
