...
import static vendor.google.common.collect.Iterables.transform;
...
final class Types {
   ...
   * vendor.google.common.reflect.TypeResolver#resolveType} will not be able to call {@code getAnnotatedBounds()} on it, but that
  ...
  enum JavaVersion {
    ...
    static {
      if (AnnotatedElement.class.isAssignableFrom(TypeVariable.class)) {
        if (new vendor.google.common.reflect.TypeCapture<Entry<String, int[][]>>() {}.capture()
            .toString()
      ...
      } else if (new vendor.google.common.reflect.TypeCapture<int[]>() {}.capture() instanceof Class) {
        CURRENT = JAVA7;
      } else {
        CURRENT = JAVA6;
      }
