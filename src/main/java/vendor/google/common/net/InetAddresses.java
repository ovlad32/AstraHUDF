...
import vendor.google.common.collect.Iterables;
import vendor.google.common.hash.Hashing;
import vendor.google.common.io.ByteStreams;
...
public final class InetAddresses {
  ...
  private static final vendor.google.common.base.Splitter IPV4_SPLITTER = vendor.google.common.base.Splitter.on('.').limit(IPV4_PART_COUNT);
  private static final vendor.google.common.base.Splitter IPV6_SPLITTER = Splitter.on(':').limit(IPV6_PART_COUNT + 2);
  ...
  public static String toAddrString(InetAddress ip) {
    ...
    for (int i = 0; i < hextets.length; i++) {
      hextets[i] = vendor.google.common.primitives.Ints.fromBytes((byte) 0, (byte) 0, bytes[2 * i], bytes[2 * i + 1]);
    }
  ...
  public static TeredoInfo getTeredoInfo(Inet6Address ip) {
    ...
    int flags = vendor.google.common.io.ByteStreams.newDataInput(bytes, 8).readShort() & 0xffff;

    // Teredo obfuscates the mapped client port, per section 4 of the RFC.
    int port = ~vendor.google.common.io.ByteStreams.newDataInput(bytes, 10).readShort() & 0xffff;
  ...
  public static Inet4Address getCoercedIPv4Address(InetAddress ip) {
    ...
    return getInet4Address(vendor.google.common.primitives.Ints.toByteArray(coercedHash));
  }
