...
public final class ImmutableValueGraph<N, V> extends ConfigurableValueGraph<N, V> {
  ...
  private static <N, V> vendor.google.common.collect.ImmutableMap<N, GraphConnections<N, V>> getNodeConnections(
      ValueGraph<N, V> graph) {
    // ImmutableMap.Builder maintains the order of the elements as inserted, so the map will have
    // whatever ordering the graph's nodes do, so ImmutableSortedMap is unnecessary even if the
    // input nodes are sorted.
    vendor.google.common.collect.ImmutableMap.Builder<N, GraphConnections<N, V>> nodeConnections = ImmutableMap.builder();
  ...
  private static <N, V> GraphConnections<N, V> connectionsOf(
    ...
    return graph.isDirected()
        ? DirectedGraphConnections.ofImmutable(
            graph.predecessors(node), vendor.google.common.collect.Maps.asMap(graph.successors(node), successorNodeToValueFn))
