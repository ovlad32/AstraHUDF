...
import vendor.google.common.base.Functions;
import vendor.google.common.graph.GraphConstants.Presence;
...
public class ImmutableGraph<N> extends ForwardingGraph<N> {
  ...
  private static <N> vendor.google.common.collect.ImmutableMap<N, GraphConnections<N, Presence>> getNodeConnections(
      Graph<N> graph) {
    // ImmutableMap.Builder maintains the order of the elements as inserted, so the map will have
    // whatever ordering the graph's nodes do, so ImmutableSortedMap is unnecessary even if the
    // input nodes are sorted.
    vendor.google.common.collect.ImmutableMap.Builder<N, GraphConnections<N, Presence>> nodeConnections = ImmutableMap.builder();
  ...
  private static <N> GraphConnections<N, Presence> connectionsOf(Graph<N> graph, N node) {
    ...
    return graph.isDirected()
        ? DirectedGraphConnections.ofImmutable(
            graph.predecessors(node), vendor.google.common.collect.Maps.asMap(graph.successors(node), edgeValueFn))
