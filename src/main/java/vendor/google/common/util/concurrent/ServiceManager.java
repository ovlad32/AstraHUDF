...
import static vendor.google.common.base.Predicates.equalTo;
import static vendor.google.common.base.Predicates.in;
import static vendor.google.common.base.Predicates.instanceOf;
import static vendor.google.common.base.Predicates.not;
import static java.util.concurrent.TimeUnit.MILLISECONDS;

import vendor.google.common.base.Function;
import vendor.google.common.base.MoreObjects;
import vendor.google.common.base.Stopwatch;
import vendor.google.common.collect.MultimapBuilder;
...
import vendor.google.common.collect.*;
...
public final class ServiceManager {
  ...
  public abstract static class Listener {
     ...
     * Service.State#RUNNING running} state. If any services fail during start up or {@linkplain
     * Service.State#FAILED fail}/{@linkplain Service.State#TERMINATED terminate} before all other services have
     * started {@linkplain Service.State#RUNNING running} then this method will not be called.
     */
    public void healthy() {}

    /**
     * Called when the all of the component services have reached a terminal state, either
     * {@linkplain Service.State#TERMINATED terminated} or {@linkplain Service.State#FAILED failed}.
     */
    public void stopped() {}

    /**
     * Called when a component service has {@linkplain Service.State#FAILED failed}.
     *
  ...
  private final vendor.google.common.collect.ImmutableList<Service> services;

  /**
   * Constructs a new instance for managing the given services.
   *
   * @param services The services to manage
   * @throws IllegalArgumentException if not all services are {@linkplain Service.State#NEW new} or if there
   *     are any duplicate services.
   */
  public ServiceManager(Iterable<? extends Service> services) {
    vendor.google.common.collect.ImmutableList<Service> copy = vendor.google.common.collect.ImmutableList.copyOf(services);
    if (copy.isEmpty()) {
    ...
    for (Service service : copy) {
      ...
      // to a NEW service.
      checkArgument(service.state() == Service.State.NEW, "Can only manage NEW services, %s", service);
    }
   ...
   * is only valid to call this method if all of the services are {@linkplain Service.State#NEW new}.
   *
   * @return this
   * @throws IllegalStateException if any of the Services are not {@link Service.State#NEW new} when the
   *     method is called.
   */
  ...
  public ServiceManager startAsync() {
    for (Service service : services) {
      ...
      checkState(state == Service.State.NEW, "Service %s is %s, cannot start it.", service, state);
    }
    for (Service service : services) {
      try {
   ...
   * will become healthy after all the component services have reached the {@linkplain Service.State#RUNNING
   * running} state.
   *
   ...
   * reached the {@linkplain Service.State#RUNNING running} state.
   *
  ...
  /**
   * Returns true if all services are currently in the {@linkplain Service.State#RUNNING running} state.
   *
   ...
   */
  public vendor.google.common.collect.ImmutableMultimap<Service.State, Service> servicesByState() {
   ...
   */
  public vendor.google.common.collect.ImmutableMap<Service, Long> startupTimes() {
  ...
  private static final class ServiceManagerState {
    final vendor.google.common.util.concurrent.Monitor monitor = new vendor.google.common.util.concurrent.Monitor();

    @GuardedBy("monitor")
    final SetMultimap<Service.State, Service> servicesByState =
        MultimapBuilder.enumKeys(Service.State.class).linkedHashSetValues().build();

    @GuardedBy("monitor")
    final Multiset<Service.State> states = servicesByState.keys();

    @GuardedBy("monitor")
    final Map<Service, Stopwatch> startupTimers = vendor.google.common.collect.Maps.newIdentityHashMap();
     ...
     */
    final vendor.google.common.util.concurrent.Monitor.Guard awaitHealthGuard = new AwaitHealthGuard();

    @WeakOuter
    final class AwaitHealthGuard extends vendor.google.common.util.concurrent.Monitor.Guard {
      AwaitHealthGuard() {
    ...
    final vendor.google.common.util.concurrent.Monitor.Guard stoppedGuard = new StoppedGuard();
    ...
    /**
     * Marks the {@link Service.State} as ready to receive transitions. Returns true if no transitions have
     * been observed yet.
     */
    void markReady() {
      monitor.enter();
      try {
        if (!transitioned) {
          ...
          List<Service> servicesInBadStates = vendor.google.common.collect.Lists.newArrayList();
        ...
        if (!monitor.waitForUninterruptibly(awaitHealthGuard, timeout, unit)) {
          throw new TimeoutException(
              "Timeout waiting for the services to become healthy. The "
                  ...
                  + vendor.google.common.collect.Multimaps.filterKeys(servicesByState, in(ImmutableSet.of(Service.State.NEW, Service.State.STARTING))));
        }
        checkHealthy();
      } finally {
        monitor.leave();
        ...
        if (!monitor.waitForUninterruptibly(stoppedGuard, timeout, unit)) {
          throw new TimeoutException(
              "Timeout waiting for the services to stop. The following "
                  ...
                  + vendor.google.common.collect.Multimaps.filterKeys(servicesByState, not(in(EnumSet.of(Service.State.TERMINATED, Service.State.FAILED)))));
        }
      } finally {
        monitor.leave();
      }
    ...
    ImmutableMultimap<Service.State, Service> servicesByState() {
      vendor.google.common.collect.ImmutableSetMultimap.Builder<Service.State, Service> builder = ImmutableSetMultimap.builder();
      monitor.enter();
      try {
        for (Entry<Service.State, Service> entry : servicesByState.entries()) {
    ...
    vendor.google.common.collect.ImmutableMap<Service, Long> startupTimes() {
        ...
        IllegalStateException exception =
            new IllegalStateException(
                "Expected to be healthy after starting. The following services are not running: "
                    + Multimaps.filterKeys(servicesByState, not(equalTo(Service.State.RUNNING))));
        for (Service service : servicesByState.get(Service.State.FAILED)) {
   ...
   * also calls the {@link ServiceManagerState#transitionService(Service, Service.State, Service.State)}, to record
