...
import static vendor.google.common.base.Throwables.throwIfUnchecked;
...
/**
 * An abstract implementation of {@link vendor.google.common.util.concurrent.ListenableFuture}, intended for advanced users only. More
 ...
 * existing one, typically using methods like {@link Futures#transform(vendor.google.common.util.concurrent.ListenableFuture,
 * Function, java.util.concurrent.Executor) Futures.transform} and {@link
 * Futures#catching(vendor.google.common.util.concurrent.ListenableFuture, Class, Function,
 ...
 * #setFuture(vendor.google.common.util.concurrent.ListenableFuture)} and {@link #setException(Throwable)}. Subclasses may also override
...
public abstract class AbstractFuture<V> extends InternalFutureFailureAccess
   ...
   */
  interface Trusted<V> extends vendor.google.common.util.concurrent.ListenableFuture<V> {}
  ...
  private static final class SetFuture<V> implements Runnable {
    ...
    final vendor.google.common.util.concurrent.ListenableFuture<? extends V> future;

    SetFuture(AbstractFuture<V> owner, vendor.google.common.util.concurrent.ListenableFuture<? extends V> future) {
      this.owner = owner;
      this.future = future;
    }
      ...
      while (true) {
        if (ATOMIC_HELPER.casValue(abstractFuture, localValue, valueToSet)) {
          ...
          if (localValue instanceof SetFuture) {
            ...
            vendor.google.common.util.concurrent.ListenableFuture<?> futureToPropagateTo = ((SetFuture) localValue).future;
  ...
  @CanIgnoreReturnValue
  protected boolean setFuture(vendor.google.common.util.concurrent.ListenableFuture<? extends V> future) {
    checkNotNull(future);
    Object localValue = value;
    if (localValue == null) {
      if (future.isDone()) {
   ...
   * SettableFuture} do not expose it to their users as an
