...
final class JdkBackedImmutableBiMap<K, V> extends ImmutableBiMap<K, V> {
  ...
  static <K, V> ImmutableBiMap<K, V> create(int n, Entry<K, V>[] entryArray) {
    Map<K, V> forwardDelegate = vendor.google.common.collect.Maps.newHashMapWithExpectedSize(n);
    Map<V, K> backwardDelegate = vendor.google.common.collect.Maps.newHashMapWithExpectedSize(n);
    for (int i = 0; i < n; i++) {
  ...
  ImmutableSet<K> createKeySet() {
    return new vendor.google.common.collect.ImmutableMapKeySet<K, V>(this);
  }
