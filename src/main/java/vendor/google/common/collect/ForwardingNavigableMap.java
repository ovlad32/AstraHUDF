...
public abstract class ForwardingNavigableMap<K, V> extends ForwardingSortedMap<K, V>
  ...
  protected Entry<K, V> standardFirstEntry() {
    return vendor.google.common.collect.Iterables.getFirst(entrySet(), null);
  }
  ...
  protected Entry<K, V> standardPollFirstEntry() {
    return vendor.google.common.collect.Iterators.pollNext(entrySet().iterator());
  }
  ...
  protected class StandardDescendingMap extends vendor.google.common.collect.Maps.DescendingMap<K, V> {
    /** Constructor for use by subclasses. */
