...
/**
 * Implementation for {@link vendor.google.common.collect.FilteredMultimap#values()}.
 *
 * @author Louis Wasserman
 */
@GwtCompatible
final class FilteredMultimapValues<K, V> extends AbstractCollection<V> {
  @Weak private final vendor.google.common.collect.FilteredMultimap<K, V> multimap;

  FilteredMultimapValues(vendor.google.common.collect.FilteredMultimap<K, V> multimap) {
    this.multimap = checkNotNull(multimap);
  }

  @Override
  public Iterator<V> iterator() {
    return vendor.google.common.collect.Maps.valueIterator(multimap.entries().iterator());
  }
  ...
  public boolean removeAll(Collection<?> c) {
    return vendor.google.common.collect.Iterables.removeIf(
        multimap.unfiltered().entries(),
        // explicit <Entry<K, V>> is required to build with JDK6
        Predicates.<Entry<K, V>>and(
            multimap.entryPredicate(), vendor.google.common.collect.Maps.<V>valuePredicateOnEntries(Predicates.in(c))));
  }
