...
import vendor.google.common.base.Equivalence;
 ...
 * appropriate {@link vendor.google.common.collect.DiscreteDomain} to {@link ContiguousSet#create}.
 *
...
public final class Range<C extends Comparable> extends RangeGwtSerializationDependencies
  ...
  static <C extends Comparable<?>> vendor.google.common.collect.Ordering<Range<C>> rangeLexOrdering() {
    return (vendor.google.common.collect.Ordering<Range<C>>) (vendor.google.common.collect.Ordering) RangeLexOrdering.INSTANCE;
  }
  ...
  public static <C extends Comparable<?>> Range<C> range(
          C lower, vendor.google.common.collect.BoundType lowerType, C upper, vendor.google.common.collect.BoundType upperType) {
    checkNotNull(lowerType);
    checkNotNull(upperType);

    Cut<C> lowerBound =
        (lowerType == vendor.google.common.collect.BoundType.OPEN) ? Cut.aboveValue(lower) : Cut.belowValue(lower);
    Cut<C> upperBound =
        (upperType == vendor.google.common.collect.BoundType.OPEN) ? Cut.belowValue(upper) : Cut.aboveValue(upper);
   ...
   * @since 14.0
   */
  public static <C extends Comparable<?>> Range<C> upTo(C endpoint, vendor.google.common.collect.BoundType boundType) {
    switch (boundType) {
      case OPEN:
   ...
   * @since 14.0
   */
  public static <C extends Comparable<?>> Range<C> downTo(C endpoint, vendor.google.common.collect.BoundType boundType) {
    switch (boundType) {
      case OPEN:
   ...
   * returned range is {@linkplain vendor.google.common.collect.BoundType#CLOSED closed} on both ends.
   *
   * @since 14.0
   */
   ...
   * given values. The returned range is {@linkplain vendor.google.common.collect.BoundType#CLOSED closed} on both ends.
   *
  ...
  public static <C extends Comparable<?>> Range<C> encloseAll(Iterable<C> values) {
    ...
    if (values instanceof SortedSet) {
      ...
      if (vendor.google.common.collect.Ordering.natural().equals(comparator) || comparator == null) {
    ...
    while (valueIterator.hasNext()) {
      ...
      min = vendor.google.common.collect.Ordering.natural().min(min, value);
      max = vendor.google.common.collect.Ordering.natural().max(max, value);
    }
  ...
  /**
   * Returns the type of this range's lower bound: {@link vendor.google.common.collect.BoundType#CLOSED} if the range includes
   * its lower endpoint, {@link vendor.google.common.collect.BoundType#OPEN} if it does not.
   *
   * @throws IllegalStateException if this range is unbounded below (that is, {@link
   *     #hasLowerBound()} returns {@code false})
   */
  public vendor.google.common.collect.BoundType lowerBoundType() {
  ...
  /**
   * Returns the type of this range's upper bound: {@link vendor.google.common.collect.BoundType#CLOSED} if the range includes
   * its upper endpoint, {@link vendor.google.common.collect.BoundType#OPEN} if it does not.
   *
   * @throws IllegalStateException if this range is unbounded above (that is, {@link
   *     #hasUpperBound()} returns {@code false})
   */
   ...
   * to preprocess ranges with {@link #canonical(vendor.google.common.collect.DiscreteDomain)}.
   */
   ...
   * #canonical(vendor.google.common.collect.DiscreteDomain)} before testing for connectedness.
   */
  ...
  private static class RangeLexOrdering extends vendor.google.common.collect.Ordering<Range<?>> implements Serializable {
