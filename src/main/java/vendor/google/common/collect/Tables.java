...
import vendor.google.common.collect.Table.Cell;
...
public final class Tables {
  ...
  public static <T, R, C, V, I extends vendor.google.common.collect.Table<R, C, V>> Collector<T, ?, I> toTable(
  ...
  private static <R, C, V> void merge(
          vendor.google.common.collect.Table<R, C, V> table, R row, C column, V value, BinaryOperator<V> mergeFunction) {
    checkNotNull(value);
   ...
   * way around.
   */
  public static <R, C, V> vendor.google.common.collect.Table<C, R, V> transpose(vendor.google.common.collect.Table<R, C, V> table) {
  ...
  private static class TransposeTable<C, R, V> extends AbstractTable<C, R, V> {
    final vendor.google.common.collect.Table<R, C, V> original;

    TransposeTable(vendor.google.common.collect.Table<R, C, V> original) {
    ...
    @Override
    public void putAll(vendor.google.common.collect.Table<? extends C, ? extends R, ? extends V> table) {
    ...
    private static final Function<Table.Cell<?, ?, ?>, Table.Cell<?, ?, ?>> TRANSPOSE_CELL =
        new Function<Table.Cell<?, ?, ?>, Table.Cell<?, ?, ?>>() {
          @Override
   ...
   *
   * <p>The views returned by the {@code Table} methods {@link vendor.google.common.collect.Table#column}, {@link
   * vendor.google.common.collect.Table#columnKeySet}, and {@link vendor.google.common.collect.Table#columnMap} have iterators that don't support {@code
   ...
   */
  @vendor.google.common.annotations.Beta
  public static <R, C, V> vendor.google.common.collect.Table<R, C, V> newCustomTable(
   ...
   * operations like {@link vendor.google.common.collect.Table#containsValue} and {@code Table.toString()}. For this to perform
   ...
   */
  @vendor.google.common.annotations.Beta
  public static <R, C, V1, V2> vendor.google.common.collect.Table<R, C, V2> transformValues(
          vendor.google.common.collect.Table<R, C, V1> fromTable, Function<? super V1, V2> function) {
  ...
  private static class TransformedTable<R, C, V1, V2> extends AbstractTable<R, C, V2> {
    final vendor.google.common.collect.Table<R, C, V1> fromTable;
    final Function<? super V1, V2> function;

    TransformedTable(vendor.google.common.collect.Table<R, C, V1> fromTable, Function<? super V1, V2> function) {
    ...
    @Override
    public void putAll(vendor.google.common.collect.Table<? extends R, ? extends C, ? extends V2> table) {
      throw new UnsupportedOperationException();
    }
   ...
   * @since 11.0
   */
  public static <R, C, V> vendor.google.common.collect.Table<R, C, V> unmodifiableTable(
      vendor.google.common.collect.Table<? extends R, ? extends C, ? extends V> table) {
    return new UnmodifiableTable<>(table);
  }

  private static class UnmodifiableTable<R, C, V> extends ForwardingTable<R, C, V>
      implements Serializable {
    final vendor.google.common.collect.Table<? extends R, ? extends C, ? extends V> delegate;

    UnmodifiableTable(vendor.google.common.collect.Table<? extends R, ? extends C, ? extends V> delegate) {
      this.delegate = checkNotNull(delegate);
    }

    @SuppressWarnings("unchecked") // safe, covariant cast
    @Override
    protected vendor.google.common.collect.Table<R, C, V> delegate() {
      return (vendor.google.common.collect.Table<R, C, V>) delegate;
    }
    ...
    @Override
    public void putAll(vendor.google.common.collect.Table<? extends R, ? extends C, ? extends V> table) {
      throw new UnsupportedOperationException();
    }
   ...
   * @since 22.0
   */
  public static <R, C, V> vendor.google.common.collect.Table<R, C, V> synchronizedTable(vendor.google.common.collect.Table<R, C, V> table) {
    return vendor.google.common.collect.Synchronized.table(table, null);
  }

  static boolean equalsImpl(vendor.google.common.collect.Table<?, ?, ?> table, @Nullable Object obj) {
    if (obj == table) {
      return true;
    } else if (obj instanceof vendor.google.common.collect.Table) {
      vendor.google.common.collect.Table<?, ?, ?> that = (vendor.google.common.collect.Table<?, ?, ?>) obj;
