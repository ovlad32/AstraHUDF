...
/**
 * Fixed-size {@link vendor.google.common.collect.Table} implementation backed by a two-dimensional array.
 *
...
public final class ArrayTable<R, C, V> extends AbstractTable<R, C, V> implements Serializable {
   ...
   */
  public static <R, C, V> ArrayTable<R, C, V> create(vendor.google.common.collect.Table<R, C, V> table) {
    return (table instanceof ArrayTable<?, ?, ?>)
        ? new ArrayTable<R, C, V>((ArrayTable<R, C, V>) table)
        : new ArrayTable<R, C, V>(table);
  }

  private final vendor.google.common.collect.ImmutableList<R> rowList;
  private final vendor.google.common.collect.ImmutableList<C> columnList;

  // TODO(jlevy): Add getters returning rowKeyToIndex and columnKeyToIndex?
  private final vendor.google.common.collect.ImmutableMap<R, Integer> rowKeyToIndex;
  private final vendor.google.common.collect.ImmutableMap<C, Integer> columnKeyToIndex;
  private final V[][] array;

  private ArrayTable(Iterable<? extends R> rowKeys, Iterable<? extends C> columnKeys) {
    this.rowList = vendor.google.common.collect.ImmutableList.copyOf(rowKeys);
    this.columnList = vendor.google.common.collect.ImmutableList.copyOf(columnKeys);
     ...
     * acknolwedge them.
     */
    rowKeyToIndex = vendor.google.common.collect.Maps.indexMap(rowList);
  ...
  private ArrayTable(vendor.google.common.collect.Table<R, C, V> table) {
  ...
  private abstract static class ArrayMap<K, V> extends IteratorBasedAbstractMap<K, V> {
    private final vendor.google.common.collect.ImmutableMap<K, Integer> keyIndex;

    private ArrayMap(vendor.google.common.collect.ImmutableMap<K, Integer> keyIndex) {
    ...
    Entry<K, V> getEntry(final int index) {
      ...
      return new vendor.google.common.collect.AbstractMapEntry<K, V>() {
        @Override
        public K getKey() {
    ...
    Iterator<Entry<K, V>> entryIterator() {
      return new vendor.google.common.collect.AbstractIndexedListIterator<Entry<K, V>>(size()) {
        @Override
   ...
   */
  public vendor.google.common.collect.ImmutableList<R> rowKeyList() {
    return rowList;
  }
  ...
  Iterator<Cell<R, C, V>> cellIterator() {
    return new vendor.google.common.collect.AbstractIndexedListIterator<Cell<R, C, V>>(size()) {
      @Override
  ...
  public Map<R, V> column(C columnKey) {
    checkNotNull(columnKey);
    Integer columnIndex = columnKeyToIndex.get(columnKey);
    return (columnIndex == null) ? vendor.google.common.collect.ImmutableMap.<R, V>of() : new Column(columnIndex);
  }
   ...
   */
  @Override
  public vendor.google.common.collect.ImmutableSet<C> columnKeySet() {
  ...
  Iterator<V> valuesIterator() {
    return new vendor.google.common.collect.AbstractIndexedListIterator<V>(size()) {
      @Override
