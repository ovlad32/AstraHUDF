...
/**
 * Implementation of {@link Multimaps#filterEntries(vendor.google.common.collect.Multimap, Predicate)}.
 *
 * @author Jared Levy
 * @author Louis Wasserman
 */
@GwtCompatible
class FilteredEntryMultimap<K, V> extends vendor.google.common.collect.AbstractMultimap<K, V> implements vendor.google.common.collect.FilteredMultimap<K, V> {
  final vendor.google.common.collect.Multimap<K, V> unfiltered;
  final Predicate<? super Entry<K, V>> predicate;

  FilteredEntryMultimap(vendor.google.common.collect.Multimap<K, V> unfiltered, Predicate<? super Entry<K, V>> predicate) {
  ...
  private boolean satisfies(K key, V value) {
    return predicate.apply(vendor.google.common.collect.Maps.immutableEntry(key, value));
  }
  ...
  static <E> Collection<E> filterCollection(
    ...
    if (collection instanceof Set) {
      return vendor.google.common.collect.Sets.filter((Set<E>) collection, predicate);
    } else {
  ...
  boolean removeEntriesIf(Predicate<? super Entry<K, Collection<V>>> predicate) {
    ...
    while (entryIterator.hasNext()) {
      ...
      if (!collection.isEmpty() && predicate.apply(vendor.google.common.collect.Maps.immutableEntry(key, collection))) {
  ...
  class AsMap extends ViewCachingAbstractMap<K, Collection<V>> {
    ...
    Set<K> createKeySet() {
      @WeakOuter
      class KeySetImpl extends vendor.google.common.collect.Maps.KeySet<K, Collection<V>> {
        KeySetImpl() {
          super(AsMap.this);
        }
        ...
        public boolean removeAll(Collection<?> c) {
          return removeEntriesIf(vendor.google.common.collect.Maps.<K>keyPredicateOnEntries(in(c)));
        }

        @Override
        public boolean retainAll(Collection<?> c) {
          return removeEntriesIf(vendor.google.common.collect.Maps.<K>keyPredicateOnEntries(not(in(c))));
        }
    ...
    Set<Entry<K, Collection<V>>> createEntrySet() {
      @WeakOuter
      class EntrySetImpl extends vendor.google.common.collect.Maps.EntrySet<K, Collection<V>> {
        @Override
            ...
            protected Entry<K, Collection<V>> computeNext() {
              while (backingIterator.hasNext()) {
                ...
                if (!collection.isEmpty()) {
                  return vendor.google.common.collect.Maps.immutableEntry(key, collection);
                }
              }
              return endOfData();
            }
    ...
    Collection<Collection<V>> createValues() {
      @WeakOuter
      class ValuesImpl extends vendor.google.common.collect.Maps.Values<K, Collection<V>> {
        ValuesImpl() {
          super(AsMap.this);
        }
        ...
        @Override
        public boolean removeAll(Collection<?> c) {
          return removeEntriesIf(vendor.google.common.collect.Maps.<Collection<V>>valuePredicateOnEntries(in(c)));
        }
  ...
  class Keys extends Multimaps.Keys<K, V> {
    ...
    public Set<Multiset.Entry<K>> entrySet() {
      return new vendor.google.common.collect.Multisets.EntrySet<K>() {
